...
// Wrapper to extract regency and pass it to Destinations component
function DestinationsContent() {
  const { regency } = useParams();
  return <Destinations hideShell forcedRegency={regency} />;
}

export default function DestinationsLanding() {
  const { language } = useTranslation();
  const navigate = useNavigate();
  const location = useLocation();

  const navGroup = NAV_GROUPS.find((g) => g.to === "/destinations");
      ...
      <div className="bg-slate-50/50 py-10" style={{ backgroundColor: "var(--bg-surface-alt)" }}>
        <div className="content-wrap">
          ...
          <p className="mb-8 max-w-2xl text-[16px]" style={{ color: "var(--text-secondary)" }}>
            {language === "id" ? "Jelajahi keindahan Yogyakarta dari berbagai kabupaten dan kota." : "Explore the beauty of Yogyakarta across its regencies and city."}
          </p>
